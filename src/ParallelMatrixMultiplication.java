...
public class ParallelMatrixMultiplication {
    ...
    ParallelMatrixMultiplication(ArrayList<ArrayList<Double>> a, ArrayList<ArrayList<Double>> b) {
        // assumption : a and b are both double[MATRIX_SIZE][MATRIX_SIZE]
        this.MATRIX_SIZE = a.size();
        ...
        for (int i = 0; i < MATRIX_SIZE; i++) {
            tempA =  new double[MATRIX_SIZE];
            tempB =  new double[MATRIX_SIZE];
            for (int j = 0; j < MATRIX_SIZE; j++) {
                tempA[j] = a.get(i).get(j);
                tempB[j] = b.get(i).get(j);
            }
            this.a[i] = tempA;
            this.b[i] = tempB;
